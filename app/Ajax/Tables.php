...
class Tables {
	...
	public function copy_table() {
		...
		if ( $id ) {
			...
			if ( $response ) {
				wp_send_json_success([// phpcs:ignore
					'message'      => sprintf( __( '%s table copied.', 'sheetstowptable' ), $response ),// phpcs:ignore
					'tables'       => $tables,
					'tables_count' => count( swptls()->database->table->get_all() ),
				]);
			}
		...
		wp_send_json_error([
			'message'      => sprintf( __( 'Invalid table to perform copied.' ), $id ),
			'tables'       => $tables,
			'tables_count' => count( swptls()->database->table->get_all() ),
		]);
	}
	...
	public function get_table_preview() {
		...
		if ( swptls()->helpers->is_pro_active() ) {
			...
			$response = swptls()->helpers->generate_html( $table_name, $settings, $table_data );
		}
